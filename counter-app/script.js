...
document.querySelector('.save-button').addEventListener('click', function() {
    ...
    if(counter == 0) {
        document.querySelector('.message').style.color = '#cfcb11';
        document.querySelector('.message').innerText = "Cannot save a zero value.";
        return;
    }
    localStorage.setItem('savedCounter', counter);
        document.querySelector('.message').style.color = 'green';
    document.querySelector('.message').innerText = `${counter} value saved to local storage successfully.`;
    document.querySelector('.counter-value').innerText = 0;
});